...
logo = "BEAST BOYx1b[1;92m ------------------------------------------\n\x1b[0;91m\x01             CREATE FILE EASILY WITH THIS COMMAND:)     Channel Subscribe Must Krna           \x1b[0m\n-----------------------------------------------\n\x1b[0;91m \x1a OWNER : Dare Devil ~ Dare Devil THE World Hacker \n\x1b[0;97m \x1a Github : DareDevil\n\x1b[0;91m \x1a Note   : Use  Kiwi Browser Token"
...
def haha():
    ...
    if ok == '':
    	...
    	os.system("xdg-open https://youtube.com/channel/UCNAeNWfXgJLaoYTIXwzxWRQ")        login()
...
def login():
    ...
    try:
        ...
        if ___token___ in ('', ' '):
            ...
            exit('%s[%s!%s]%sYAAR TOKEN LAGA ' % (P, M, P, M))

        xwx = requests.get('https://graph.facebook.com/me/?access_token=%s' % ___token___).json()

        print '%s[%s*%s]%s WelCome To Devil World %s %s' % (B, P, B, P, H, xwx['name'].lower())
...
def menu():
    ...
    print '\x1b[1;91mExample: /sdcard/Devil.txt'
